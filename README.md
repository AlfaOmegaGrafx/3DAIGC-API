...
* **Template VRM rig** — `rig_mode: template` on `POST /api/v1/auto-rigging/generate-rig` applies `operator-local template_ict.vrm` skeleton to AIGC meshes (Blender pipeline in `scripts/blender/`).
...
Photo → TRELLIS mesh → **ICT** template rig → optional VRM download + TripoSplat preview. Full design, blend-shape roadmap, and rig alignment notes: **[docs/AVATAR_PIPELINE.md](docs/AVATAR_PIPELINE.md)** (wrap: [MESH_WRAP_ROADMAP.md](docs/MESH_WRAP_ROADMAP.md), Arc2Avatar: [ARC2AVATAR_TRACK.md](docs/ARC2AVATAR_TRACK.md)).
...
| **Template VRM** (`rig_mode: template`) | Mesh | Rigged GLB | — | Applies ICT humanoid template skeleton to AIGC mesh (bones-only; see [docs/AVATAR_PIPELINE.md](docs/AVATAR_PIPELINE.md)) |
