...
# Optional preflight: HF pins + TRELLIS.2 conditioning guards (skip with P3D_SKIP_PREFLIGHT=1)
if [[ "${P3D_SKIP_PREFLIGHT:-0}" != "1" ]]; then
    echo "🔍 Preflight (venv drift + HF conditioning)..."
    if ! bash scripts/check_venv_drift.sh; then
        echo "❌ HF venv drift — run: ./venv/bin/pip install -c scripts/constraints-hf.txt transformers==4.57.3"
        echo "   Or skip once: P3D_SKIP_PREFLIGHT=1 ./scripts/run_server.sh"
        exit 1
    fi
    if ! ./venv/bin/python scripts/verify_env_compat.py; then
        echo "❌ Preflight failed — see messages above"
        echo "   Full GPU HF check: P3D_HF_VERIFY_GPU=1 ./venv/bin/python scripts/verify_hf_conditioning.py --gpu"
        exit 1
    fi
    echo "✅ Preflight OK"
    echo ""
