...
This file is the **archived MindLink full protocol** previously in root `CLAUDE.md`.
RepoResident (`CLAUDE.md` + `.agent/`) is now the primary engineering harness.
Keep using `.brain/` for durable personal/project memory when MindLink MCP is available.
